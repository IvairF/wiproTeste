...
namespace Amil.AsmWebCad.Utils
{
    public static class DynamicQueryableHelper
    {
        ...
        public static IQueryable<T> OrderByNew<T>(this IList<T> source, string ordering, params object[] values)
        {
            return (IQueryable<T>)OrderBy(source.AsQueryable(), ordering, values);
        }
        ...
        public static IQueryable<T> OrderByMultiLevel<T>(this IEnumerable<T> source, string orderByValues)
        {
            return OrderByMultiLevel(source.AsQueryable(), orderByValues);
        }

        public static IQueryable<TEntity> OrderByMultiLevel<TEntity>(this IQueryable<TEntity> source, string orderByValues)
        {
            var orderPair = orderByValues.Trim().Split(',')[0];
            var orderPairUpper = orderPair.ToUpper();
            var command = orderPairUpper.EndsWith(" DESC") ? "OrderByDescending" : "OrderBy";
            ...
            if (propertyName.Contains('.'))
            {
                ...
                for (var i = 1; i < childProperties.Length; i++)
                {
                    ...
                    property = !t.IsGenericType ? t.GetProperty(childProperties[i]) : t.GetGenericArguments().First().GetProperty(childProperties[i]);
            ...
            var returnValue = source.Provider.CreateQuery<TEntity>(resultExpression);

            if (orderByValues.Trim().Split(',').Length <= 1) return returnValue;
            var newSearchForWords = orderByValues.Remove(0, orderByValues.IndexOf(',') + 1);
            return source.OrderByMultiLevel(newSearchForWords);
        }
    ...
    internal class Signature : IEquatable<Signature>
    {
        ...
        public bool Equals(Signature other)
        {
            ...
            return !Properties.Where((t, i) => other != null &&
...
#pragma warning restore 0219
        ...
        private void ValidateToken(TokenId t, string errorMessage)
        {
            ...
            if (!Enum.IsDefined(typeof(TokenId), t))
                throw new InvalidEnumArgumentException("t", (int) t, typeof(TokenId));
        }
